...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "text") == 0 || strcmp(argv[1], "image") == 0) {
        property_data result = {0};
        bool received = read_clipboard(strcmp(argv[1], "image") == 0, &result);
        assert(!received);
    ...
    while ((event = xcb_wait_for_event(connection))) {
        ...
        if (type == XCB_SELECTION_REQUEST) {
        ...
        } else if (type == XCB_PROPERTY_NOTIFY && !sent_chunk) {
            ...
            if (notification->window == requestor && notification->atom == property &&
                ...
                xcb_flush(connection);
                sent_chunk = true; // Never send the remaining data or the terminating empty chunk.
            }
        }
        free(event);
    }
